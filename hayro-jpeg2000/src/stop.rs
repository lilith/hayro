...
//!
//! [`StopCheck`] is a single self-contained type following the
//! zero-dependency pattern described in
//! <https://github.com/imazen/enough/blob/main/ZERO-DEP.md>: a clone-cheap
//! handle wrapping an optional user closure that the decoder polls at coarse
//! intervals (once per tile and once per code block). [`StopCheck::none()`]
//! costs nothing to poll, so it can be threaded through internals
//! unconditionally.
...
impl StopCheck {
    ...
    /// Polls the underlying source. [`StopCheck::none()`] always returns
    /// false.
